...
public class AviaSouls {
    ...
    public Ticket[] search(String from, String to) {
        ...
        // Сортируем массив result по цене в порядке возрастания
        Arrays.sort(result);
        return result;
    }

    public void sortTickets() {
        Arrays.sort(tickets); // метод для сортировки билетов
    }

    public Ticket[] searchAndSortBy(String from, String to, Comparator<Ticket> comparator) {
        Ticket[] result = search(from, to); // Ищем билеты по маршруту
        Arrays.sort(result, comparator); // Сортируем их с помощью переданного компаратора
        return result;
    }
}
